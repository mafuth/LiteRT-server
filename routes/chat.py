...
async def chat_completions(request: Request, body: ChatCompletionRequest):
    ...
    async with request.app.state.llmEngine.lock:
        engine = request.app.state.llmEngine.getEngine()
    ...
    if body.stream:
        def stream_generator():
            try:
                ...
                with engine.create_conversation(messages=history) as conversation:
                    ...
                    token_ids = engine.tokenize(completion_text)
                    num_tokens = len(token_ids)
                    
                    # TPS based on generation time (from first token)
                    gen_time = end_time - first_token_time if first_token_time else total_elapsed
                    tps = num_tokens / gen_time if gen_time > 0 else 0
                    
                    logger.info(f"Performance: TTFT={ttft:.4f}s, TPS={tps:.2f}, Total={total_elapsed:.4f}s, Tokens={num_tokens}")
                    
                    # Include stats in a final chunk
                    stats_chunk = {
                        "id": f"chatcmpl-{uuid.uuid4()}",
                        "object": "chat.completion.chunk",
                        "created": int(time.time()),
        ...
        try:
            ...
            with engine.create_conversation(messages=history) as conversation:
                ...
                logger.info(f"Performance: TPS={tps:.2f}, Total={total_elapsed:.4f}s, Tokens={num_tokens}")
